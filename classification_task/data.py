...
def random_sample_negative(a, b, c, d):
    """Randomly sample 8 invalid analogies from the process."""
    list_of_forms=[(a__, b__, c__, d__)
        for a_, b_, c_, d_ in enrich(a, b, c, d) # generate 8 positive (valid) analogies
        for a__, b__, c__, d__ in generate_negative(a_, b_, c_, d_) # for each positive (valid) analogy generate 3 negative (invalid) analogies
    ]
    
    # random sampling
    return random.sample(list_of_forms, 8)
