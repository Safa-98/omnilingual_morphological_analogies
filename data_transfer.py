...
class Task1Dataset(torch.utils.data.Dataset):
	...
	def __init__(self, source_language="german", target_language="german", word_voc = [], mode="train", word_encoding="none"):
		super(Task2Dataset).__init__()
	...
	def set_analogy_classes(self):
		self.analogies = []
		self.all_words_source = set()
		self.all_words_target = set()
		#print(len(self.raw_data_source))
		for i, (word_a_i, feature_b_i, word_b_i) in enumerate(self.raw_data_source):
			self.all_words_source.add(word_a_i)
			self.all_words_source.add(word_b_i)
			for j, (word_a_j, feature_b_j, word_b_j) in enumerate(self.raw_data_target):
				self.all_words_target.add(word_a_i)
				self.all_words_target.add(word_b_i)
				if feature_b_i == feature_b_j:
					self.analogies.append((i,j))
			#if not i%1000:
			#	print(word_a_i, feature_b_i, word_b_i)
		#print(len(self.analogies), " analogies in ", self.mode, " mode")
	def encode_word(self, word, voc):
		if self.word_encoding == "char":
			return torch.LongTensor([voc[c] if c in voc.keys() else -1 for c in word])
	...
	def encode(self, a, b, c, d):
		return self.encode_word(a, self.word_voc_id), self.encode_word(b, self.word_voc_id), self.encode_word(c, self.word_voc_id), self.encode_word(d, self.word_voc_id)
	...
	def __getitem__(self, index):
		...
		a, feature_b, b = self.raw_data_source[ab_index]
		c, feature_d, d = self.raw_data_target[cd_index]
		return self.encode(a, b, c, d)
	def get_vocab(self):
		return set(w for w1w2 in [(w1, w2) for w1,feature,w2 in self.raw_data_source] for w in w1w2)
